...
<xsl:variable name="mypreds" select="document('probable_features.xml',/)/probablefeatures/aspect[position() = $index]/feature"/>
  ...
  <xsl:template match="/">
    <xsl:variable name="title">
      ...
      <xsl:text> ): Probable Profiles</xsl:text>
    </xsl:variable>  
    <xsl:variable name="imagetitle">
      <xsl:text>aspect_</xsl:text>
      <xsl:value-of select="$index"/>
      <xsl:text>_probable_features.png</xsl:text>
    </xsl:variable>
    <html>
      <body>
        <div id="frontmatter">
