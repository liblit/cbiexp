...
void init_options(configuration *c)
{
  ...
  assert(c->token_window_size <= TOKEN_WINDOW_SIZE_MAX);	
...
void handle_options(int argc, char *argv[])
{
	...
	if (STREQ(argv[i],"-t")) {
            ...
            if ((config.token_window_size < 1) || (config.token_window_size > TOKEN_WINDOW_SIZE_MAX))
	       {
	          fprintf(config.errfile,"Error: Tile size of %d is out of range.  Setting tile size to default of %d\n", config.tile_size, TOKEN_WINDOW_SIZE_MAX-1);
	          config.token_window_size = TOKEN_WINDOW_SIZE_MAX;
	          config.tile_size = config.token_window_size - 1;
	       }
            continue; 
        }
	if (STREQ(argv[i],"-w")) {
           config.winnowing_window_size=  atoi(argv[++i]); 
           if ((config.winnowing_window_size< 1) || (config.winnowing_window_size> WINNOWING_WINDOW_SIZE_MAX))
	       {
	           fprintf(config.errfile,"Error: Winnowing window size of %d is out of range.  Setting winnowing window to default of %d\n", config.winnowing_window_size, WINNOWING_WINDOW_SIZE_MAX);
	           config.winnowing_window_size= WINNOWING_WINDOW_SIZE_MAX;
	       }
           continue; 
        }
