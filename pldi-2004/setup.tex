...
\placeholder{[Alice] ``Do well'' is not very precise.  Do you mean
  ``design correctly''?  I've changed the wording slightly}
%While all software
...
are called \termdef{databases}.  We modified the code to remove the check for a
...
\placeholder{[Alice] It would be clearer if we commented the above
  code to explain why we are matching on '\\n'.  Readers unfamiliar
  with bug \#1 may be confused.}
...
We removed many such sources of coupling among input parameters that we
