...
null \texttt{FILE} pointer in the case that the database cannot be opened
for writing.  This bug is analogous to one reported in \ccrypt.  This
is a deterministic bug, and in fact the program crashes almost
immediately after failing to open the file.
...
loaded, the pointer to the end of one array \texttt{A} was not moved to
reflect that new data had been added to the end of \texttt{A}. The
...
of the first database stored in \texttt{A}.  This bug has unpredictable
...
\texttt{flex}.  While examining predicates on \texttt{flex}'s internal state
