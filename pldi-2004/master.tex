...
\newcommand{\exifpred}{{\tt o + s > buf\_size is TRUE}}
...
\issue{modified by Mike (``The most general technique...'')}
...
\issue{Modified by Mike (deleted ``the problem persists...'')}
...
\issue{Modified by Mike (deleted ``..based on different techniques and offering...'')}
...
\issue[Mike]{"run-time values to be sound" -> "run-time values to be found" (?)
...
\issue{Modified by Mike: "Our predicates, however,..." -> "Our predicates do, however,..."}
...
\issue{Modified by Mike: "uses the multivariate state estimation technique"
->
"uses multivariate state estimation techniques"}
