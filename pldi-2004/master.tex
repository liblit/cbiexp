...
\issue[Alice]{``use post-deployment'' doesn't make sense}
...
Each $(\mathtt{y}_i, \mathtt{c}_j)$ pair
%% compared-to $\mathtt{y}_i$ or $\mathtt{c}_j$
is treated as a distinct instrumentation site.
  %%
\end{description}

All predicates at a given site are updated jointly.  For
example, sampling a single negative return
value yields true observations of the $< 0$, $\leq 0$, and $\neq 0$ predicates and
false observations of the $> 0$, $\geq 0$, and $= 0$ predicates
at that site.  Also, note that in any scheme, the logical negation of every
predicate at any given site is itself a predicate at that site.
...
predicates do, however, give coverage information: the sum of all predicate counters at a site reveals the relative coverage of that site.
...
\issue[Alice]{It's not correct to say that the crash stacks weren't
  useful at all for \exif, is it?}
