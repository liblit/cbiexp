...
returns a positive value.  This function is part of the GTK, an 
object system for C.  Objects in the the GTK can be referred to both
...
Much later clean-up code dutifully calls {\tt g\_source\_remove} on the
...
and nulling fields (thus the predicate above) in the belief that the objects are freed. However, there
...
These bugs were by the most complex of any that we dealt with in any
of our experiments (although one or two of the \moss\ bugs are close).  The
...
that our techniques did not point us as directly to the bug as one might hope.
...
strategies focus on low-level properties of C code, while \rhythmbox\
is really written in a higher-level language, namely at the level of
the GTK's API.  We have already identified a new instrumentation strategy
...
would still be widely applicable, as the GTK is widely used.  We expect
that such system- or API-specific instrumentation strategies would be
needed in many cases to obtain the maximum benefit from our techniques.
...
deal of interest in applying static analysis to software quality.  Some,
...
executions for its data, is that it is possible to assign an accurate importance to each
bug.
