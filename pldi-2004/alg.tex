...
\item Identify the cause of the most important bug $\bug$.

\item Fix $\bug$, and repeat.

\end{enumerate}

\issue[Alice]{Modified to differentiate between a bug and a bug
  profile}

For our purposes, identifying the cause of a bug $\bug$ means selecting a
...
\issue[Alice]{Changed P or not P to ``P observed,'' because according
  to our notation, P is the name of a predicate, NOT the value of the
  predicate.  We've been using R(P) to denote the value of the
  predicate in run R.}
This leads us to the following definition:
\[
\context(P) \equiv \prob(\fail = 1 | P \mbox{ observed})
\]
Now, it is not the case that $P$ is observed in every run, because the
site where this predicate occurs may not reached in every run.
...
\[ \context(P) = \frac{F(P \mbox{ observed})}{S(P \mbox{ observed}) +
  F(P \mbox{ observed})} \]
...
\begin{align*}
  \hat \pi_f(P) &= \frac{F(P)}{F(P \mbox{ observed})} &
  \hat \pi_s(P) &= \frac{S(P)}{S(P \mbox{ observed})}
\end{align*}
...
let $a = F(P)$, $b = S(P)$, $c = F(P\mbox{ observed})$, and $d =
S(P\mbox{ observed})$.
Then
\begin{gather*}
...
We use a concise \termdef{bug thermometer} to visualize the information
...
\issue[Alice]{Modified to differentiate between bugs and bug profiles.}
