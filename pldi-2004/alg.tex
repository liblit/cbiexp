...
But on line {\tt (c)} we have $\crash(\mbox{\tt x == 0}) = 1.0$ as well.
...
reaches line {\tt (c)} at all, the crash value of {\tt x == 0} is 1.0.

As the predicate {\tt x == 0} after line {\tt (c)} of the example
...
In any given run, neither of $P$ or $\neg P$ may be observed (because the sampled where this predicate is
...
result.  We need step (2) because while $\increase(P)$ is very good at
...
Finally, it should be clear that the algorithm is very efficient.
