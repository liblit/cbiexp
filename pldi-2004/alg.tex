\placeholder{We need to make sure we mention somewhere that we
transform our counters into real predicates before running this
algorithm.  We have made the point that this is possible in
\Autoref{sec:background}, but I [Ben] feel that stating we do this is
properly part of the algorithm discussion.}

\placeholder{[Alice] I took out all the $\#$ signs in the formulas
  because they make the formulas unnecessarily clunky.  Does the
  current form look acceptable to everyone?}
...
\placeholder{[Alice] I think this example needs to be made more
  explicit.  By {\tt if (random)}, do you mean an event that
  may or may not happen?  so that {\tt f} may become a valid pointer
  with some probability less than 1, and may remain an invalid pointer
  otherwise?  It may be clearer to say

  \begin{quote}
if (something) f = \ldots some valid points\ldots;

else { /* f remains invalid */ }
...
\placeholder{[Alice] Have we already talked about how to tell
  whether or not a site is sampled by summing over all the predicate
  counts at that site?}
...
\placeholder{ I observe that in downsampled data we have ``causes'' that are well-ranked based on only 1 failing run.  I think we should discard these, too.}
...
\placeholder{[Alice]  The motivation for this ranking function is not
  very convincing\ldots}
...
answer.  The {\it Z test} makes use of the statistic $ Z =
...
simplified version of the {\it Z test} for two populations of trial
