...
\issue[Alice]{Prob (P => Crash) should really be something like Pr (Crash = 1 | P = 1) [text modified]}
\issue[Mike]{I've never liked the notation "Prob(P \implies Crash)".  What
this quantity really is is "Prob(Crash | P = true)".  The notation
"Prob(P \implies Crash)" doesn't suggest a conditional probability.
Failure(P) really is a conditional probability, and I think that it
...
%%
\issue[Alice]{Same comment as R(P).  S(P) and F(P) are not really
  functions, and it would be more natural to use the subscript
  notation.  [didn't modify text]}
%%
...
\issue{Modified by Mike: replaced ``surprising'' with ``unplesant''}
...
%\end{gather*}
%%
\issue[Alice]{The notation 'P v not P' confuses P with R(P).  We've
  defined P to be the name of a predicate, and R(P) to be its actual
  value in run R.  Here, we should at least say 'R(P) v not R(P)', but
  I think it's even clearer to say 'P is observed'.  Also the '=>'
  implication notation belongs more in logic than in statistics.  I've
  changed this to the conditional probability notation.}
%%
...
%%
\issue[Mayur]{Change "(because the site where this predicate is
  sampled is not reached)" to "(because the site where this predicate
  occurs is not reached, or is reached but not sampled)"?}
%%
...
\issue{Modified by Mike: "Since Increase() is a statistic, computing its confidence
interval is a well-understood problem" -> 
"Since Increase() is a statistic, computing a confidence interval 
for the underlying parameter is a well-understood problem" 
[confidence intervals are for parameters which are being estimated, 
not for statistics.]}
...
\issue{Modified by Mike: "Pruning predicates using Increase(P) \leq 0"
-> "Pruning predicates based on Increase(P)"
[Since we're using confidence intervals, the former statement
...
\issue[Alice]{The name 'Moss' should be typeset in special font.  [I
  can't find the place where this table is inserted, so I can't make
  the change myself.]}

\issue[Mayur]{Caption of Table 1(b): change ``lower bound of
  Increase(P)'' to ``Increase(P)'' since we don't explain in the text
  what "lower bound" is.  (Alternatively, it should be ``lower bound of
  the confidence interval of Increase(P)'').}

\subsection{Ranking Predicates: Specificity vs. Sensitivity}
\label{sec:ranking}

\issue[Alice]{I thought we were going to use the sensitivity vs.
  specificity terminology instead of precision vs. recall?}

\issue[Mike]{I actually *don't* think that the unnormalized score (the
  one without the log(NumF)) makes sense.  The harmonic mean is a
  combination of some notion of specificity (aka, precision) and some
  notion of sensivitity (aka, recall).  These notions have to be
  measured on commensurate scales to be combined.  Now, a raw measure
  of specificity would be Failure(P).  We're using a recentered
  measure in which Context(P) has been subtracted away.  Both
  Failure(P) and Context(P) are conditional probabilities, and their
  difference, Increase(P), is thus in general a number between -1 and
  1 (but between 0 and 1 for the predictors we're interested in).  A
  raw measure of sensitivity would be F(P)/N, where N is the total
  number of failures (Ben's "NumF").  This number also lies between 0
  and 1, and thus its inverse is reasonably combined with the inverse
  of Increase(P).  On the other hand, the quantity log F(P), and the
  quantity F(P), lie between 0 and infinity (assuming F(P)>0), and its
  inverse is *not* reasonably combined with the inverse of
  Increase(P).

  The easiest-to-justify quantity is of course

  2 / (1 / Increase(p) + N / F(P)).

  But we decided at some point that in measuring sensitivity (recall)
  it would be better to put the raw counts on a transformed scale.
  This is fine---it's always OK to consider scales other than the
  original scale for measurement.  Letting \phi() denote the nonlinear
  scaling transform, we're using \phi(F(P)) to measure "transformed
  counts".  Now we need to normalize this quantity in order to combine
  it with Increase.  A naive thought would be to normalize using
  \sum_P \phi(F(P)).  But this doesn't take into account that the P
  overlap in their coverage.  A better thing to do is to do what Ben
  did:  use \phi(N).  This insures that the sensitivity (recall)
  scores lie between 0 and 1, and that a predicate which is always on
  will have a score of 1; perfect sensitivity.

  I.e., I would write the formula:

  2 / (1 / Increase(p) + \phi(N) / \phi(F(P))),

  Or possibly:

  2 / (1 / Increase(p) + 1 / (\phi(F(P)) / \phi(N))) ,

  where the "/" between \phi(F(P)) and \phi(N) is an explicit "/", and
  the other "/"'s are achieved via "\frac".  This is more symmetric
  although it may be uglier.

  (I'd suggest using the "phi" symbol, because the spirit of what we
...
\issue[Alice]{Why use the word "disproportionately"?  The harmonic
  mean just rewards high scores in both dimensions, and has the
  averaging affect of trading off one with the other, no?}
\issue{Modified by Mike: "selectivity" -> "sensitivity"}
\issue[Mike]{Tables 1(a) and (b) illustrate the difficulty of defining
...
%% It is helpful to consider this problem in the language of information
...
%% the corresponding terms are {\em sensitivity} and {\em specificity}.  In both fields,
...
transformation.
%%
\issue[Alex]{It might be a bit unclear since the quantity that is in
  the interval 0-1 is

  log(F(P))/log(NumF)

  and the formula as written has the reciprocal of that.}
\issue[Mike]{Replace precision by specificity and recall with sensitivity 
throughout paper. (done)}
...
\issue{Modified by Mike: "these predicates are related hiding other, distinct bugs..."
->
"these predicates are related, hiding other, distinct bugs..."}
