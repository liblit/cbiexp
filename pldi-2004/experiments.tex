...
function {\tt exif\_mnote\_data\_canon\_load} as a strong bug
...
\item the function {\tt exif\_mnote\_data\_canon\_save} passes the
  uninitialized {\tt n->entries[i]->data} to {\tt memcpy} on line \texttt{(c)}, which reads it and eventually crashes.
\end{itemize}
...
a couple of minutes and without any explicit specification beyond
...
We intend to explore schemes that instrument predicates on the heap structure in
future work.
% It required several hours to isolate each of the two bugs, in part because \rhythmbox
...
\issue[Alice]{Moved the following subsection from section 5 to section 4}
...
{\tt p + \ldots} are all sub-bug predictors of bug \#1 (see
