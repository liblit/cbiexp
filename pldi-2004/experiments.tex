...
studies, but for many of the bugs this number is clearly far more than
the minimum required. In this section, we estimate how many runs are
required for the bug predictors to be isolated.

Our estimates are computed using the following methodology.
We chose the predictor of each bug identified
in the case studies; where the elimination algorithm selected two
predictors for a bug, we picked the more natural one (i.e., not the sub-bug
predictor).  For each chosen predictor $P$, we then computed
...
where the threshold $0.2$ is selected because we observed that 
...
In these
experiments, the number of runs $N$ ranges over the values $100, 200, \ldots, 1,000, $ $2,000, 
\ldots, 25,000$.  
For each study, we list two numbers for each bug with predictor $P$:
the minimum number of runs $N$ such that the threshold test is met, and the value of
$F(P)$, the number of failing runs among those $N$ runs where $P$ is
...
we can isolate all of the bug predictors with only between 10 and 40 observations of
...
true is a fraction $0 \leq p \leq 1$ of all runs,
then $N = F/p$ runs will be required by our algorithm.
