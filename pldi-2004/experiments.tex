...
described in Section~\ref{sec:algorithm} in five case studies.\footnote{
The data can be found at FIX ME URL.}
Figure~\ref{tab:exps} shows the summary statistics for each of the
...
$\frac{1}{100}$ and both are reached once per run.  Then even though
...
but never less than $\frac{1}{100}$.  Thus, rarely executed code has a
much higher sampling rate than very frequently executed code; a
similar strategy has been pursued for similar reasons in \cite{Trishul}.  We
...
Table~\ref{tab:mossdilute} shows the results of the experiment.  The
...
The table at the far right of Table~\ref{tab:mossdilute} shows for
...
Section~\ref{sec:introduction}, more than one bug may occur in a run;
...
the other bugs.\footnote{The peculiar eighth predicate, {\tt f $<$ f}, 
...
We analyzed {\tt ccrypt} version 1.2, which has a known bug \cite{Selinger:2003:cqual}.  
The results are shown in Table~\autoref{tab:views-ccrypt}.
...
The results for \bc\ are shown in Table~\autoref{tab:views-bc}, which has a known buffer overrun.
...
Table~\autoref{tab:views-exif} shows results for \exif, an open source image processing program.
...
Table~\autoref{tab:views-rhythmbox} shows our results for \rhythmbox, an open source jukebox player.
...
(see the discussion in Section~\ref{sec:introduction}).  These bugs
