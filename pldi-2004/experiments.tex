...
\issue{Modified by Mike: "Sampling can have serious negative effects on our algorithm"
->
"Sampling creates additional challenges that must be faced by our algorithm"}
...
\issue{Modified by "Thus, rarely executed code has a much higher sampling rate
than very frequently executed code; a similar strategy has been pursued
for similar reasons in [?]."
->
"Thus, rarely executed code has a much higher sampling rate than 
very frequently executed code.  (A similar strategy has been pursued
for similar reasons in [?].)"}
...
\issue{modified by Alice, check for accuracy}
...
%%
\issue[Alice]{"we choose a sampling rate for P to achieve ..." was
  confusing. [text modified, please check for accuracy]}

\subsection{A Validation Experiment}

\issue{Modified by Mike: "an experiment where we knew" -> "an experiment in which we knew" }
...
%%
\issue[Alice]{"The nature of the bugs varies: ... comprise the eight
  crashing bugs."  was a bit awkward.  I've moved the phrase "comprise
  the eight crashing bugs" up into the first phrase.  See if you like
  it better.}

\issue{modified by Alice}
\issue[Mike]{The paragraph beginning with "The last bug..." doesn't read very
well to me.  I think that the purpose should be introduced at the outset.
If people agree, I'm happy to take a crack at a rewrite.}
...
runs.
%%
\issue[Alice]{"While the failure can be detected... " was also
  awkward.  I've rearranged things a little bit.}
...
bug ${\cal B}$ occurs and $P$ is observed to be true.  
%%
\issue[Alice]{Confusion of notation: we had defined B to be the set of
  runs causes by a specific bug, but now we're using B to denote the
  name of the bug, AND the associated set of runs.  One way to fix
  this would be to let $B$ denote the bug, and {\cal B} the set of
  runs causes by B.  Then the above sentence could be "and each bug
  $B$ the size of $|{\cal B}|$."  [haven't modified text]}
\issue[Mike]{"the number of runs where bug {\cal B} occurs" 
This is inconsistent with the definition of a bug on the first
page, which is a set of runs.  I'd suggest just dropping the
"{\cal B}" in both places in which it occurs in this sentence.}
%%
...
\issue{Modified by Mike: "its sub-bug at position two" -> "its companion sub-bug predictor
at position two" }
...
that it is related to some predicate listed earlier (i.e., its companion
sub-bug predictor 
...
\issue{Modified by Mike: "a sub-bug predictor of the second" -> "a sub-bug predictor 
...
\issue{Modified by Mike: "We could not have even begun to understand" -> 
"Note, however, that we could not have even begun to understand"}
...
\issue[Mayur]{Saying that we were unfamiliar with rhythmbox probably
  ...
  for future work?  Of course, we will retain the positive sentence
  that is currently the last sentence of this para:
