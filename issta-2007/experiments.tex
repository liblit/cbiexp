...
\begin{table}[tb]
  ...
  \caption{Number of buggy application variants in experiments, and
    ...
    predictor (at a sampling Rate of 1)}
  \label{tab-top-pred}
\end{table}
...
\autoref{sec-sampling} explains that the chance of observing a complex predicate reduces quadratically with the sampling rate.  Despite this low probability, the top scoring predictor is a conjunction in 84\% of the Siemens applications except \prog{tcas} at a sampling rate of $\nicefrac{1}{100}$.  In \prog{tcas}, there is no looping or recursion and every function is called atmost twice.  The number of observations of a predicate in \prog{tcas} is atmost two.  As a result, the chance of observing a complex predicate at lower sampling rates reduces drastically.  Only 25\% of the \prog{tcas} experiments had a complex predicate as the top predictor.
